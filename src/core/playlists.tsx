...
export interface PlayListData {
    playlistName: string;
    playlistSongData: RootObjectFav[];
}
...
function Playlists(props: updateFunction) {
    const [playListData, setPlayListData] = useState<PlayListData[]>([]);
    const [loader, setLoader] = useState(true);

    function updateCurrentPlayingSongDetails(value: number = 0, mainIndex: number = 0): void {
    ...
    useEffect(() => {
        SongsDataBaseService.getPlayListSongsDetailsBackend().then((value) => {
            ...
            setPlayListData(value as PlayListData[]);
            setLoader(false);
        })
    }, []);
